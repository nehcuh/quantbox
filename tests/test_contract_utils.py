...
class TestParseContract:
    ...
    def test_parse_czce_3digit(self):
        ...
        assert info.symbol == "SR2501"  # 郑商所保持大写
    ...
    def test_parse_invalid_futures_format(self):
        """测试解析无效期货代码格式"""
        with pytest.raises(ValueError, match="期货合约代码格式无效|合约代码.*错误，无法解析"):
...
class TestFormatContract:
    ...
    def test_format_to_goldminer(self):
        """测试转换为掘金格式"""
        # 上期所：小写
        assert format_contract("SHFE.rb2501", ContractFormat.GOLDMINER) == "SHFE.rb2501"
        # 大商所：小写
        assert format_contract("DCE.m2501", ContractFormat.GOLDMINER) == "DCE.m2501"
        # 上期能源：小写
        assert format_contract("INE.sc2409", ContractFormat.GOLDMINER) == "INE.sc2409"
        # 广期所：小写
        assert format_contract("GFEX.lc2405", ContractFormat.GOLDMINER) == "GFEX.lc2405"
        # 中金所：大写
        assert format_contract("CFFEX.IF2412", ContractFormat.GOLDMINER) == "CFFEX.IF2412"
        # 郑商所：大写+3位月份数字
        assert format_contract("CZCE.SR2501", ContractFormat.GOLDMINER) == "CZCE.SR501"

    def test_format_to_tushare(self):
        """测试转换为 Tushare 格式"""
        result = format_contract("SHFE.rb2501", ContractFormat.TUSHARE)
        assert result == "RB2501.SHF"  # Tushare 使用大写品种代码

    def test_format_to_tushare_czce(self):
        """测试转换郑商所合约为 Tushare 格式"""
        result = format_contract("CZCE.SR2501", ContractFormat.TUSHARE)
        assert result == "SR2501.ZCE"  # Tushare 使用大写品种代码

    def test_format_to_joinquant(self):
        """测试转换为聚宽格式"""
        result = format_contract("SHFE.rb2501", ContractFormat.JOINQUANT)
        assert result == "RB2501.XSGE"  # 聚宽也使用大写品种代码
...
class TestFormatContracts:
    ...
    def test_format_list(self):
        ...
        assert results == ["RB2501.SHF", "M2505.DCE"]  # Tushare 使用大写品种代码

    def test_format_comma_separated_string(self):
        """测试转换逗号分隔的字符串"""
        contracts = "SHFE.rb2501, DCE.m2505, CZCE.SR2509"
        results = format_contracts(contracts, ContractFormat.TUSHARE)
        assert results == ["RB2501.SHF", "M2505.DCE", "SR2509.ZCE"]  # Tushare 使用大写品种代码
    ...
    def test_format_single_string(self):
        ...
        assert results == ["RB2501.SHF"]  # Tushare 使用大写品种代码
...
class TestGetUnderlying:
    ...
    def test_get_futures_underlying(self):
        ...
        assert get_underlying("CZCE.SR2509") == "SR"  # 郑商所保持大写
...
class TestNormalizeContract:
    ...
    def test_normalize_czce_3digit(self):
        ...
        assert result == "CZCE.SR2501"  # 郑商所保持大写
...
class TestNormalizeContracts:
    ...
    def test_normalize_list(self):
        ...
        assert results == ["SHFE.rb2501", "DCE.m2505", "CZCE.SR2509"]  # 郑商所保持大写
...
class TestIntegration:
    ...
    def test_round_trip_conversion_czce(self):
        """测试郑商所往返转换（标准->Tushare->标准）"""
        original = "CZCE.SR2501"
        tushare = format_contract(original, ContractFormat.TUSHARE)
        back_to_standard = normalize_contract(tushare)
        assert back_to_standard == original
